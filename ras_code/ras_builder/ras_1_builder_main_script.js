"use strict";
/*  
Ras - Clip and Synthetic Voices: A tool to practice language comprehension
Antonio Cigna 2023
...
//===============================
var selected_languageRegion = "";
var selected_voiceName      = "";
let voices ;
//----------------------
window.speechSynthesis.onvoiceschanged = () => {
	voices = window.speechSynthesis.getVoices();		 
	let voiceSelect = document.getElementById("id_voices");		  
	voices.forEach((voice, i) => (voiceSelect.options[i] = new Option(voice.lang + " " + voice.name, i)));		  
}           
//-------------
function builder_get_oneLangVoice(this1) {
	console.log("builder_get_oneLangVoice(this1=" + this1.value); 
	var ix = this1.value; 
	if ( isNaN(ix) ) ix = 0; 
	if (!voices) return; 
	selected_languageRegion = voices[ix].lang; 
	selected_voiceName   	= voices[ix].name;  
	document.getElementById("id_myLang"     ).innerHTML = selected_languageRegion; 
	document.getElementById("id_myVoiceName").innerHTML = selected_voiceName; 
	document.getElementById("id_lang2").innerHTML = "  " + selected_languageRegion + " " + selected_voiceName; 
} // 
//-------------------------------------
function builder_get_language(this1) {
	if (this1.value == "no") {
		document.getElementById("id_lang2").innerHTML = ""; 
		selected_languageRegion = ""; 
		selected_voiceName   	= "";  
		document.getElementById("id_div_subt").style.display = "block"; 
		return; 
	}
	// if this1.value = "yes" then selected_lang must not be empty
	if (selected_languageRegion == "") {
		builder_get_oneLangVoice( document.getElementById("id_voices") );		
	}
	if (selected_languageRegion == "") return; 
	
	document.getElementById("id_div_subt").style.display = "block"; 
	
} // end of builder_get_language()
...
this_file_root = get_this_file_path(this_file_root); // eliminate this folder (ras_builder)

var this_ras_base = this_file_root;
...
//-----------------------------------------------------
var ras_LOCALSTOR_key = "RAS_Builder";
...
function get_titleTWO() {
    ...
    out_html_file = "ras_1_" + INTRAFN + ".html";
    script_lev2_fn = "ras_2_" + INTRAFN + ".jstxt";
    //
...
function fun_html_file_download() {
    ...
    html_fn_download = "ras_1_" + INTRAFN + ".html";
	
	html_fn_download   = correctFileName(  html_fn_download   );    
	script_fn_download = correctFileName(  script_fn_download );    

    // function second_level_html() is in file ras_1_builder_3_lev2_html_model.jsHTML
	
    var text1 = second_level_html(HTML_TITLE, LS_video_path, video_filename, this_ras_base, html_fn_download, script_fn_download);
...
function write_script3(intrafn, TXTorig, TXTtran) {
    ...
    var ras_base = this_ras_base;
    if (ras_base.indexOf("file:///") == 0) {
        ras_base = ras_base.substring(8);
    }
    var hscr3 = "var x_main_code_folder = String.raw`" + ras_base + "`;	\n";
    ...
    hscr1 += 'var x_language      = "'      + id_language         + '";  // 2 character user interface language ' + '\n';	
	hscr1 += 'var x_selected_langRegion= "' + selected_languageRegion + '";// selected language-region' + '\n';	
	hscr1 += 'var x_selected_voiceName = "' + selected_voiceName + '";     // selected synthetic voice name ' + '\n';	
	...
	**/
    ...
    //var script_sub_fn = "ras_2_" + intrafn +  ".jstxt"; 
    var script_sub_fn = "ras_1_" + intrafn + ".jstxt";
...
function initial_from_localStorage_values() {
    ...
    var stored_ras_localStor = JSON.parse(localStorage.getItem(ras_LOCALSTOR_key)); //get them back

    if (stored_ras_localStor) {
        [
            LS_video_path
        ] = stored_ras_localStor;
    } else {
        set_localStorage_var();
    }
...
function set_localStorage_var() {
    ...
    fun_console("set_localStorage_var()  key=" + ras_LOCALSTOR_key);

    var ras_LOCALSTOR_val_list = [
        LS_video_path
    ];


    localStorage.setItem(ras_LOCALSTOR_key, JSON.stringify(ras_LOCALSTOR_val_list)); //store colors

    var key = ras_LOCALSTOR_key;
